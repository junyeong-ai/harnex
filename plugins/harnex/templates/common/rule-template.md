...
<!-- harnex-fill: one bullet per invariant the code under these paths ALREADY
     ...
     `[file: path/to/file.ext:42]`, or `[file: path/to/file.ext]` where the
     whole file is the owner. Where the owner is another document, name the
     section — `[file: .claude/rules/other.md § Escape hatch]` — which fails
     on the rename that invalidates it, where a line only ever proved the file
     was that long.
