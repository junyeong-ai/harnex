...
/// relative on every platform (no leading `/`, no drive colon), forward-slash
/// separated, no traversal, no glob metacharacters, no NUL. The shape both
/// `governs.live_truth` and `[[evidence.advisories]]` inputs are held to — a
/// declaration that could reach outside the project, or that two evaluators
/// could read differently, is refused at the boundary.
